...
export function MarketingNav({ transparent = false }: MarketingNavProps) {
  ...
  return (
    <nav className={`fixed left-0 right-0 top-0 z-50 w-full px-4 sm:px-6 py-4 backdrop-blur-xl transition-colors ${transparent ? 'bg-dark-slate/80' : 'bg-dark-slate border-b border-distill-purple/30'}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <a href="/" className="hover:opacity-80 transition-opacity flex items-center gap-2 sm:gap-3">
          <Image src="/favicon.svg" alt="distill" width={32} height={32} className="w-7 h-7 sm:w-8 sm:h-8" />
          <span className="text-lg sm:text-xl font-bold text-cream">distill</span>
        </a>
        ...
        <div className="hidden lg:flex items-center gap-8">
          ...
          <div className="flex items-center gap-3 ml-2">
            ...
            <a href="/#quick-start" onClick={(e) => handleSmoothScroll(e, '#quick-start')} className="bg-gradient-to-r from-distill-purple to-distill-violet hover:from-distill-violet hover:to-distill-lavender text-white px-6 py-2 rounded-lg shadow-lg shadow-distill-purple/30 text-sm font-semibold transition-all">
              Get Started
            </a>
          </div>
        </div>

        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden p-2 text-cream hover:text-distill-purple transition-colors" aria-label="Toggle menu">
          {mobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
        </button>
      </div>

      {mobileMenuOpen && (
        <div className="lg:hidden fixed top-[72px] left-0 right-0 bg-dark-slate border-b border-distill-purple/30 backdrop-blur-xl shadow-xl z-40">
          <div className="px-4 py-6 space-y-4">
            ...
            <div className="pt-4 space-y-3 border-t border-distill-purple/20">
              ...
              <a href="/#quick-start" onClick={(e) => handleSmoothScroll(e, '#quick-start')} className="flex items-center justify-center bg-gradient-to-r from-distill-purple to-distill-violet hover:from-distill-violet hover:to-distill-lavender text-white px-6 py-3 rounded-lg shadow-lg shadow-distill-purple/30 text-sm font-semibold transition-all w-full">
                Get Started
              </a>
            </div>
          </div>
...
export function MarketingFooter() {
  ...
  return (
    <footer className="bg-dark-slate border-t border-distill-purple/30 py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-12 mb-12">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <Image src="/favicon.svg" alt="distill" width={32} height={32} className="w-8 h-8" />
              <span className="text-xl font-bold text-cream">distill</span>
            </div>
