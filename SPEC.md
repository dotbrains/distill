...
Same pluggable architecture as [`prr`](https://github.com/smeltery/prr). All AI providers implement a single interface:
...
The agent interface and provider registry follow the same pattern as `prr`. This is intentional — the two tools solve different problems but share the same AI interaction layer. A future shared module (`github.com/smeltery/agentkit`) could deduplicate this, but premature extraction is worse than duplication.
